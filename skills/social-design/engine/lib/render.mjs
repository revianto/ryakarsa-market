...
import { bleedX, cellFormat, cellPosition, getMosaicFormat, uploadOrder } from './mosaic.mjs';
...
// A mosaic deck is one master canvas (cells arranged in a grid) sliced into N
// posts — the grid gutter on Instagram's profile is what makes this a puzzle
// feed. Cells render with NO per-post chrome (wordmark/handle/swipe): unlike a
// carousel, a viewer sees the grid as one continuous surface under one profile
// header, so repeating the wordmark per tile would look broken, not branded.
export async function renderMosaicDeck({ brandPath, brand, deck, patternFiles, patternMeta, outDir, scale, log = () => {} }) {
  const mosaic = getMosaicFormat(deck.format);
  const pxScale = scale ?? brand.formats?.[deck.format]?.scale ?? mosaic.scale;
  const tokensCss = path.join(brandPath, 'tokens.css');
  if (!fs.existsSync(tokensCss)) throw new Error(`Missing ${tokensCss} — run: social tokens ${brand.name}`);

  for (const cell of deck.cells) {
    const span = patternMeta[cell.pattern]?.span || 1;
    if (span > 1) throw new Error(`Mosaic cells must be span-1 patterns — "${cell.pattern}" spans ${span}`);
  }
  ...
  const written = [];
  try {
    const page = await browser.newPage({ viewport: { width: mosaic.width, height: mosaic.height }, deviceScaleFactor: pxScale });
    await page.goto(pathToFileURL(path.join(ENGINE_DIR, 'shell.html')).href);
    const fontsUrl = googleFontsUrl(brand.fonts);
    let fontsSheetError = null;
    if (fontsUrl) {
    ...
    fs.mkdirSync(outDir, { recursive: true });
    for (const f of fs.readdirSync(outDir)) if (/^cell-.*\.png$/.test(f)) fs.unlinkSync(path.join(outDir, f));
    const faces = requiredFaces(brand.fonts);
    const ctx = {
      brand: { name: brand.name, wordmark: brand.wordmark, handle: brand.handle },
      type: { displayWeight: 700, bodyWeight: 400, ...(brand.type || {}) },
      ...
      mosaic: { cols: mosaic.cols, rows: mosaic.rows, cellWidth: mosaic.cellWidth, cellHeight: mosaic.cellHeight,
        width: mosaic.width, height: mosaic.height, cellFormat: cellFormat(mosaic) },
    };
    await page.evaluate(({ cells, ctx }) => window.UC.renderMosaic(cells, ctx), { cells: deck.cells, ctx });
    ...
    deck.cells.forEach((cell, i) => {
      const { row, col } = cellPosition(i, mosaic);
      const standardFile = path.join(outDir, `cell-r${row}c${col}.png`);
      const bleedFile = path.join(outDir, `cell-r${row}c${col}-bleed.png`);
      cells.push({ index: i, row, col, standardFile, bleedFile });
    });

    for (const c of cells) {
      await page.screenshot({ path: c.standardFile,
        clip: { x: c.col * mosaic.cellWidth, y: c.row * mosaic.cellHeight, width: mosaic.cellWidth, height: mosaic.cellHeight } });
      written.push(c.standardFile);
      log(`OK   ${c.standardFile}`);

      await page.screenshot({ path: c.bleedFile,
        clip: { x: bleedX(c.col, mosaic), y: c.row * mosaic.cellHeight, width: mosaic.cellWidth + 80, height: mosaic.cellHeight } });
      written.push(c.bleedFile);
      log(`OK   ${c.bleedFile} (bleed — use this one for upload, crop to fit)`);
    }

    // Instagram inserts new posts top-left; upload bottom-right -> top-left, or the
    // grid comes out mirrored. This can't be automated (it's a manual IG upload
    // action), so it's surfaced here for the CLI to print.
    const order = uploadOrder(cells.length).map((i) => cells[i]);
    log(`\nUrutan upload (kanan-bawah -> kiri-atas), pakai file *-bleed.png:`);
    order.forEach((c, n) => log(`  ${n + 1}. cell-r${c.row}c${c.col}-bleed.png`));

    return { files: written, scale: pxScale, format: mosaic, uploadOrder: order };
  } finally {
    await browser.close();
  }
}
